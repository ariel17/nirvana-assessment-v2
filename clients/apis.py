...
def get_API1(member_id: int) -> Response:
    ...
    """
    return _get_api(configs.API1_MUST_FAIL, member_id, 1000, 10000, 5000)
...
def get_API2(member_id: int) -> Response:
    ...
    :return: A Response instance with specific values.
    """
    return _get_api(configs.API2_MUST_FAIL, member_id, 1200, 13000, 6000)
...
def get_API3(member_id: int) -> Response:
    ...
    :return: A Response instance with specific values.
    """
    return _get_api(configs.API3_MUST_FAIL, member_id, 1000, 10000, 6000)


def _get_api(must_fail: bool, member_id: int, deductible: int, stop_loss: int, oop_max: int) -> Response:
    if must_fail:
